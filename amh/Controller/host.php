...
class host extends AmysqlController
{
	...
	function vhost()
	{
		...
		if (isset($_GET['run']))
		{
			...
			$cmd = "amh $m $g $domain";
			if($m == "host") {
				if($g == "stop") $host_nginx = 0;
				else $host_nginx = 1;
				$sql = "update users set host_nginx = ".$host_nginx." WHERE domain='".$domain."'";
				$this -> hosts -> _query($sql);
			}
		...
		if (isset($_GET['backup']))
		{
			...
			if(!empty($domain))
			{
				$result = $this -> hosts -> host_backup_ssh($domain);
				if (strpos($result, '[OK]') !== false)
				{
					$this -> status = 'success';
					$this -> top_notice = $domain . ' : 备份虚拟主机成功。';
				}
				else {
					if(strlen($result) > 100) {
						$result = substr($result,0,100)."...";
					}
					$this -> top_notice = $domain . ' : 备份虚拟主机失败。' . $result;
				}
			}
		}
		...
		if (isset($_POST['save']))
		{
			...
			{
				$dotcount = substr_count($_POST['domain'],".");
				if ($dotcount >= 2) {					
					...
					if (strpos($result, '[OK]') !== false)
					{
						...
						$this -> status = 'success';
						$this -> notice = $_POST['domain'] . ' : 新增虚拟主机成功。';
						$_POST = array();
					}
					else
						$this -> notice = $_POST['domain'] . ' : 新增虚拟主机失败。' . $result;
				} else {
					$this -> notice = '请填写正确域名。';
				}
			}
